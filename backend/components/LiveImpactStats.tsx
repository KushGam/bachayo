...
export function LiveImpactStats() {
  return (
    <div className="mx-auto mt-14 grid max-w-[900px] grid-cols-1 gap-px overflow-hidden rounded-[24px] border border-white/20 bg-white/20 px-0 sm:grid-cols-3">
      {STATS.map((card) => (
        <div key={card.label} className="bg-[#D85A30] px-5 py-8 text-center md:py-10">
          ...
          <div className="mt-3 text-xs text-white/80 md:text-sm">{card.label}</div>
        </div>
      ))}
    </div>
  );
