...
function launchEmailHtml(cityLabel: string) {
  const place = cityLabel === 'Nepal' ? 'Nepal' : cityLabel;
  const shareText = encodeURIComponent(
    `LastBag is live in ${place}! Find rescue bags from restaurants near you at up to 70% off. Check it out at lastbag.app 🛍`,
  );
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (typeof body.city === 'string' && body.city.trim()) {
      cityParam = body.city.trim();
    }
  } catch {
    // Empty body → send to all (legacy clients).
  }

  const cityFilter =
    cityParam !== 'all' ? normalizeWaitlistCity(cityParam) : null;
  if (cityParam !== 'all' && !cityFilter) {
  ...
  if (waitlist.length === 0) {
    return NextResponse.json(
      {
        error: cityFilter
          ? `No waitlist emails found for ${cityFilter}`
          : 'No waitlist emails found',
      },
      { status: 400 },
    );
  }
